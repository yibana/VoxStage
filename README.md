...
- **`vox-engine`（crates/engine）**  
  - 执行引擎层，负责：
    ...
    - 从 DSL AST 中收集 `role` 定义、解析 `speak` / `sleep` 语句。
    - 构造 `SynthesisRequest`，调用对应的 `TtsProvider`，返回 `AudioStream`。
...
- **`vox-cli`（crates/cli）**  
  - 命令行入口示例程序，用于将各层能力串联起来：
    ...
    - 从 `.vox` 脚本文件读取 DSL 源码，调用 `vox-engine::run_script_with_dsl` 执行。
    - 遍历执行结果中的 `AudioStream`，调用 `vox-audio` 进行播放。
...
- ✅ `vox-dsl`：
  - 支持 `model` / `role` / `speak` / `sleep` 语法：
    ...
    - `sleep` 语句：在执行过程插入延迟（毫秒）：
      - `sleep 1000  # 延迟 1 秒`
  - 输出独立于执行的 AST 结构（`Script` / `Item` / `ModelDef` / `RoleDef` / `SpeakStmt` / `SleepStmt`）。

- ✅ `vox-engine`：
  - 提供 `ModelManager` 持有 `TtsProvider` 实例。
  - `run_script_with_dsl(manager, src)`：
    - 解析 DSL 源码 → AST。
    - 收集所有 `role` 定义为运行时配置。
    - 顺序执行：
      - `speak`：构造 `SynthesisRequest`，调用绑定模型的 Provider。
      - `sleep`：调用 `tokio::time::sleep` 延迟指定毫秒数。
    - 返回 `(模型名称, AudioStream)` 列表，由上层决定如何播放或处理音频。
...
speak Girl "你好，这是的第一个示例。"
sleep 1000
speak Girl(speed = 1.3) "这句语速更快"
sleep 1000
speak Girl(speed = 1) "这句语速更更快"
```

- 运行效果：
  - 第一句以角色默认参数播放。
  - `sleep 1000` 让后一句在 1 秒后播放。
  - 第二、三句通过 `speak(... )` 覆盖 `speed` 参数，体现语速变化。

---

## 后续规划（MVP → 完整架构）

- [ ] 为 DSL 增加 `let` 变量、字符串插值、条件与循环控制流。
- [ ] 在 `vox-engine` 中引入变量作用域、表达式求值以及更智能的调度。
