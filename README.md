...
- **`voxstage-gui`（apps/voxstage-gui）**  
  - Tauri 2 + Vue 3 桌面应用，规划为剧本编辑器：
    - **Phase 1 已完成**：应用壳、顶部栏 + 主内容区布局、前后端桥接验证。
    - **后续**：全局配置（模型 / 角色）、列表式剧本编排（缩进子列表表示 if/for/while）、与 engine/runner 对接运行。详见 `docs/gui-phase1.md`。
...
- 剧本编辑器为 Tauri 桌面应用，当前为 Phase 1 骨架（布局 + 桥接验证）：
...
- 需已安装 Node.js 与 pnpm。窗口打开后主内容区会显示 Rust 桥接测试文案。后续 Phase 将加入全局配置与列表式剧本编排。
...
- [ ] **GUI 剧本编辑器**：Phase 2 全局配置（模型/角色）、Phase 3 列表式编排（缩进子列表）、Phase 4 与 engine/runner 对接运行。
